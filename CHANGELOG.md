...
## 2026.04.27.19 — 输入输出字数限制，节省积分约 20-25%

### Behavior

- **Step 1 新增问题截断（300 字）**：
  - 提取问题后若超过 300 字，自动截取前 300 字 + 末尾加 `…（已截断）`
  - 不询问用户，直接截断——超长问题通常含不必要的背景，核心论题 300 字足够表达
  - R2/R3/R4 里所有 `{user_question}` 占位符都会因此变短，连锁节省

- **Step 3 R1 prompt 加输出字数限制（600 字）**：
  - 原来 `Be thorough but concise`（无约束）→ 现在明确 `≤ 600 characters (Chinese) / ≤ 600 words (English)`
  - 附加要求：优先核心论点，省略展开举例/背景铺垫/重复表达，每句话带新信息
  - 选用 600 字而非 400 字：保留技术题 / 对比题的必要空间，避免强制省略关键维度
  - R2 prompt 里带 3 家完整 R1 答案 → R1 变短后 R2 input 自动减少约 30%，**连锁效应最大**

- **不改 R3 rebuttal**：R3 已有 `200–400 words is usually ideal`，与 R1 的 600 字协调一致，保持现状

### 节省估算

- R1 单家：~5-15 积分 → ~4-10 积分（约 -25%）
- R2 单家（连锁）：~10-25 积分 → ~7-16 积分（约 -30%）
- **moco 全程：~250-700 积分 → ~180-480 积分（约 -25%）**
...
- **`README.md`**：repo 主页的面向同事的入口文档
  - 核心是一句话安装：跟 WorkBuddy 说 `帮我装 MoneyMoneyGo/moco-skill`
  - 触发示例（`moco:` / `模型 PK:` / `几个 AI 一起答:` / `多模态对比:` 等）
  - 消耗参考速览（~250-700 积分/次 + 省积分贴士）
  - 文件结构表（让同事快速知道 repo 里什么在哪）
  - 不堆冗长文档，保持"新用户 30 秒看完就能用"的密度

### 设计决策

- 不加"📢 推荐给同事"按钮、不做 zip release 自动化、不做 workbuddy:// 自定义协议——现有的 `npx skills add <owner/repo>` 机制已经足够支持同事一键安装，无需额外工程化
- README 引用 GitHub repo 主页作为唯一权威入口，不复制 SKILL.md 里的细节（避免双份维护）

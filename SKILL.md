...
| 4 | DeepSeek V3.2 | `#7C3AED` | DeepSeek | `deepseek-v3-2-volc-ioa` |

> **2026.04.27.5 起 GLM-4.7 已被 DeepSeek-V3.2 替换**。原因：GLM-4.7 的多模态能力在不同 Agent 调用间不稳定（18:48 真实事故记录在 V3 校验日志里），且 DeepSeek-V3.2 在严密推理 + 找茬挑战场景的口碑更强、视觉读图实测稳定（20:22 探针通过）。
...
- **默认锁定阵容（写死）**：Round 1/2/3 必须分别使用 `claude-sonnet-4.6-1m`、`gpt-5.4`、`gemini-3.0-pro`、`deepseek-v3-2-volc-ioa`；Round 4 裁判必须使用 `claude-opus-4.7-1m`。
...
Repeat for the other 3 models with their respective IDs (`gpt-5.4`, `gemini-3.0-pro`, `deepseek-v3-2-volc-ioa`).
...
**Model assignment**: Each rebuttal MUST be generated by **the defender's own model** — pass `model="<defender's WorkBuddy model ID>"` (e.g. when DeepSeek V3.2 is rebutting, use `model="deepseek-v3-2-volc-ioa"`).
...
要点速览：
- 企业版 WorkBuddy 月配额 = **10 万积分 / 人**（约 $1000，1 积分 ≈ $0.01）
- moco 单次约等于 **~250-700 积分**（≈ 25× 单次质询）
- 主智能体编排成本"自动跟随会话当前模型"，切 Sonnet 4.6 1M 即可减半
- 国产 ioa 模型（DeepSeek / GLM / Hunyuan）当前实测 **0 积分**
